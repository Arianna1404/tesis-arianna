...
# ============================================
# MVDR como estimador de DOA (PHI only)
# ============================================

@jit(nopython=True, fastmath=True, cache=True)
def fast_mvdr_power(phi, Rinv, theta_fixed, f, mic_positions, c):
    """Potencia de salida del MVDR beamformer para una dirección phi"""
    a = fast_steering_vector_phi_only(theta_fixed, phi, f, mic_positions, c)
    denom = (a.conj().T @ Rinv @ a).real
    if denom < 1e-12:
        return 0.0
    return 1.0 / denom   # <-- esta es la función de "pseudo-espectro" MVDR


def optimized_mvdr_phi_only(R, theta_fixed=90.0, fs=16000):
    """
    Estimación de azimut usando MVDR beamformer como función de DOA
    Retorna: (phi_estimado, confianza) o (None, 0.0)
    """
    if R is None or R.size == 0 or R.shape[0] != R.shape[1]:
        return None, 0.0

    # Grid angular (más denso en frontal, más grueso en trasera)
    ...
    # Regularización (proporcional al ruido promedio por canal)
    epsilon = 1e-4 * np.trace(R) / R.shape[0]
    R_reg = R + epsilon * np.eye(R.shape[0], dtype=R.dtype)

    # Inversión con fallback
    try:
        Rinv = np.linalg.inv(R_reg)
    except np.linalg.LinAlgError:
        print("Advertencia: matriz singular en MVDR → usando pseudoinversa")
        Rinv = np.linalg.pinv(R_reg)

    # Frecuencias de análisis (típico para voz / binaural)
    freqs = np.linspace(2000, 6000, 20)

    pseudo_spectrum = np.zeros(len(phi_range), dtype=np.float64)

    # Función interna para paralelizar por frecuencia
    def compute_power_at_freq(f):
        ps = np.zeros(len(phi_range), dtype=np.float64)
        for j, phi in enumerate(phi_range):
            a = fast_steering_vector_phi_only(theta_fixed, phi, f, mic_positions, c)
            denom = np.real(a.conj() @ Rinv @ a)
            if denom > 1e-10:           # evita división por cero o valores negativos
                ps[j] = 1.0 / denom
        return ps

    # Paralelización (si tienes muchas frecuencias vale la pena)
    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=4) as executor:
        ps_list = list(executor.map(compute_power_at_freq, freqs))

    pseudo_spectrum = np.sum(ps_list, axis=0)

    # Normalización suave
    ps_max = np.max(pseudo_spectrum)
    if ps_max > 1e-8:
        pseudo_spectrum_norm = pseudo_spectrum / ps_max
    else:
        pseudo_spectrum_norm = pseudo_spectrum  # todo muy bajo → se queda en 0

    # ────────────────────────────────────────────────
    # Detección de pico – parámetros típicos para MVDR
    # ────────────────────────────────────────────────
    from scipy.signal import find_peaks

    peaks, properties = find_peaks(
        pseudo_spectrum_norm,
        height=0.15,           # MVDR tiene picos menos pronunciados que MUSIC
        distance=5,            # ~5–8° separación mínima
        prominence=0.06,       # ayuda a descartar picos muy planos
        width=None
    )
    ...
    # Elegimos el pico con mayor altura
    best_idx = peaks[np.argmax(properties['peak_heights'])]
    phi_est = phi_range[best_idx]

    # Normalizamos a [-180, 180)
    phi_est = (phi_est + 180) % 360 - 180

    # Cálculo de confianza (heurística adaptada a MVDR)
    peak_val = pseudo_spectrum_norm[best_idx]
    mean_val = np.mean(pseudo_spectrum_norm)
    std_val  = np.std(pseudo_spectrum_norm)

    snr_like = (peak_val - mean_val) / (std_val + 1e-10)
    confidence = np.clip(snr_like / 3.5, 0.0, 1.0)   # divisor más bajo que en MUSIC
...
def stabilize_phi_tolerante(phi_raw, confidence, phi_prev):
    ...
    coherence_factor = np.exp(-angular_dev / 30.0)
...
def localize_source_optimized(mic1, mic2, mic3, mic4, fs):
    ...
    # 1. Filtrado primero (necesario para calcular R)
    with ThreadPoolExecutor(max_workers=4) as executor:
    ...
    # 2. Matriz de covarianza del frame actual
    ...
    # 4. Si no hay nada en el historial → no podemos estimar
    if len(R_history) == 0:
        return None, 0.0, 0.0, 0.0

    # 5. Covarianza promedio (aquí se crea R)
    R = np.mean(R_history, axis=0)

    # 6. Ahora sí podemos estimar DOA
    phi_raw, conf = optimized_mvdr_phi_only(R, theta_fixed=THETA_FIXED, fs=fs)

    if phi_raw is None or conf <= MIN_CONFIDENCE:
        return None, 0.0, 0.0, 0.0

    # Aplicar mapeo (si lo necesitas para MVDR también)
    ...
    # Usar grafo como veto
    if score < GRAPH_SCORE_THRESHOLD and phi_prev is not None:
        phi_final = phi_prev
    else:
